const STATUS_STYLES = [
  ...
  { match: ["probation", "đang thử việc", "trial"], className: "badge-probation", label: "Probation" },
  { match: ["inactive", "terminated", "resigned", "nghỉ việc"], className: "badge-inactive", label: "Inactive" },
];
...
export const getStatusPresentation = (rawStatus) => {
  ...
  const matched = STATUS_STYLES.find((item) => item.match.some((keyword) => normalized.includes(keyword)));
  if (!matched) {
